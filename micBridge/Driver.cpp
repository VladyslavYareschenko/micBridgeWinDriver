...
DRIVER_DISPATCH KsDeviceControl;

PDRIVER_DISPATCH OriginalDeviceControlHandler;
...
)
{
	...
	//
	OriginalDeviceControlHandler = DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL];
...
)
{
	...
	switch (stack->Parameters.DeviceIoControl.IoControlCode)
	{
		...
		{
			if (auto* microphone = AdapterCommon::getInstance().getMicrophone())
			{
				KSSTREAM_HEADER* header = static_cast<KSSTREAM_HEADER*>(stack->Parameters.DeviceIoControl.Type3InputBuffer);

				microphone->writeToAllStreams((PBYTE)header->Data, header->FrameExtent, false);
			}
		}
		default:
			return OriginalDeviceControlHandler(_DeviceObject, _Irp);
	}
}
