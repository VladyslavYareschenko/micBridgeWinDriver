...
{
	PAGED_CODE();
	DPF_ENTER(("[AdapterCommon::CreateAudioInterfaceWithProperties]"));
...
#pragma warning( disable : 4706 )
		if (0 <= status)
		{
			...
			{
				ACCESS_MASK Access;
				KeyToCreate = wcschr(++ObjectName.Buffer, '\\');
				if (KeyToCreate)
				{
				...
				{
					return bool(ObjectName.Buffer = KeyToCreate);
				}());
		}
#pragma warning( pop )
...
(
	...
	_In_			BOOLEAN									WriteSymbolicLinkToReg
)
{
	/*
	...
	*/
	...
	if (NT_SUCCESS(ntStatus))
	{
		if (WriteSymbolicLinkToReg)
		{			
			_DbgPrintF(DEBUGLVL_VERBOSE, ("[SaveMixerSettingsToRegistry]"));

			{
				HANDLE hKey;
				CreateKey(&hKey, KEY_ALL_ACCESS,
					L"\\REGISTRY\\MACHINE",
					L"\\SOFTWARE\\MicBridge\\MicIO", 0);
			}
			...
			RtlInitUnicodeString(&KeyName, L"\\REGISTRY\\MACHINE\\SOFTWARE\\MicBridge\\MicIO");
			InitializeObjectAttributes(&ObjectAttributes, &KeyName, OBJ_CASE_INSENSITIVE, NULL, NULL);

			// open the driver registry key
			NTSTATUS ntStatusLoc = PcNewRegistryKey(&DriverKey,               // IRegistryKey
				NULL,                     // OuterUnknown
				GeneralRegistryKey,        // Registry key type
			...
			if (NT_SUCCESS(ntStatusLoc))
			{
				KeyName;

				// make a unicode strong for the subkey name
				RtlInitUnicodeString(&KeyName, L"Data");

				// open the settings subkey
				ntStatusLoc = DriverKey->NewSubKey(&SettingsKey,              // Subkey
					NULL,                      // OuterUnknown
					KEY_ALL_ACCESS,            // Access flags
					&KeyName,                  // Subkey name
					REG_OPTION_NON_VOLATILE,   // Create options
				...
				if (NT_SUCCESS(ntStatusLoc))
				{
					// loop through all mixer settings
						RtlInitUnicodeString(&KeyName, L"MicBridgeIOSymlink");

						// set the key
						ntStatusLoc = SettingsKey->SetValueKey(&KeyName,                 // Key name
							REG_LINK,                // Key type
							symbolicLink.Buffer,
							symbolicLink.Length);
...
{
	PAGED_CODE();
	DPF_ENTER(("[AdapterCommon::ConnectTopologies]"));
...
{
	PAGED_CODE();
	DPF_ENTER(("[AdapterCommon::DisconnectTopologies]"));
