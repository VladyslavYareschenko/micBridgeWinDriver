...
)
{
	...
	case KSSTATE_RUN:
		...
		m_RingBuffer->Clear();
		//lockFreeBuff_.producerClear();
		//lockFreeBuff_.consumerClear();
...
NTSTATUS MiniportWaveRTStream::WriteAudioPacket(BYTE* buffer, ULONG packetSize, BOOL eos)
{
	...
	// if (packetSize > 1024 * 24) return STATUS_BUFFER_TOO_SMALL;

	NTSTATUS state = m_RingBuffer->Put(buffer, packetSize);
	//NTSTATUS state = lockFreeBuff_.writeBuff(buffer, packetSize) < packetSize ? 
...
--*/
{
	...
	while (ByteDisplacement > 0)
	{
		...
		// actuallyWritten = lockFreeBuff_.readBuff(m_pDmaBuffer + bufferOffset, runWrite);
