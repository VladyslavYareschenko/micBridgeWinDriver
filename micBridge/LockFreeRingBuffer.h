...
public:
    ...
    void consumerClear(void) {
        tail.store<kmdf_std::memory_order_relaxed>(head.load<kmdf_std::memory_order_relaxed>());
    }
    ...
    index_t readAvailable(void) const {
        return head.load(index_acquire_barrier) - tail.load(kmdf_std::memory_order_relaxed);
    }
    ...
    index_t writeAvailable(void) const {
        return buffer_size - (head.load(kmdf_std::memory_order_relaxed) - tail.load(index_acquire_barrier));
    }
    ...
    bool insert(T data)
    {
        index_t tmp_head = head.load(kmdf_std::memory_order_relaxed);
        ...
        else
        {
            ...
            kmdf_std::atomic_signal_fence(kmdf_std::memory_order_release);
    ...
    bool remove()
    {
        index_t tmp_tail = tail.load(kmdf_std::memory_order_relaxed);

        if (tmp_tail == head.load(kmdf_std::memory_order_relaxed))
            return false;
        else
    ...
    size_t remove(size_t cnt) {
        index_t tmp_tail = tail.load(kmdf_std::memory_order_relaxed);
        index_t avail = head.load(kmdf_std::memory_order_relaxed) - tmp_tail;
    ...
    T& operator[](size_t index) {
        return data_buff[(tail.load(kmdf_std::memory_order_relaxed) + index) & buffer_mask];
    }
...
private:
    ...
    constexpr static kmdf_std::memory_order index_acquire_barrier = fake_tso ?
        kmdf_std::memory_order_relaxed
        : kmdf_std::memory_order_acquire; // do not load from, or store to buffer before confirmed by the opposite side
    constexpr static kmdf_std::memory_order index_release_barrier = fake_tso ?
        kmdf_std::memory_order_relaxed
        : kmdf_std::memory_order_release; // do not update own side before all operations on data_buff committed

    alignas(cacheline_size) kmdf_std::atomic<index_t> head; //!< head index
    alignas(cacheline_size) kmdf_std::atomic<index_t> tail; //!< tail index
    ...
    //static_assert(kmdf_std::numeric_limits<index_t>::is_integer, "indexing type is not integral type");
    //static_assert(!(kmdf_std::numeric_limits<index_t>::is_signed), "indexing type must not be signed");
    //static_assert(buffer_mask <= ((kmdf_std::numeric_limits<index_t>::max)() >> 1),
    //    "buffer size is too large for a given indexing type (maximum size for n-bit type is 2^(n-1))");

    //static_assert(kmdf_std::is_trivial<T>::value, "non trivial objects will currently break");
};
...
size_t Ringbuffer<T, buffer_size, fake_tso, cacheline_size, index_t>::writeBuff(const T* buff, size_t count)
{
    ...
    kmdf_std::atomic_signal_fence(kmdf_std::memory_order_release);
    head.store<index_release_barrier>(tmp_head);

    return to_write;
    //index_t available = 0;
    //index_t tmp_head = head.load<kmdf_std::memory_order_relaxed>();
    //int to_write = static_cast<int>(count);
    //size_t written = 0;

    //available = buffer_size - (tmp_head - tail.load<index_acquire_barrier>());

    //if (available < count) // do not write more than we can
    //    to_write = static_cast<int>(available);

    //size_t buffer_offset = tmp_head % buffer_size;

    //while (to_write > 0)
    //{
    //    size_t run_write = min(to_write, buffer_size - buffer_offset);
    //    RtlCopyMemory(data_buff + buffer_offset, buff + written, run_write);
    //    buffer_offset = (buffer_offset + run_write) % buffer_size;
    //    to_write -= static_cast<int>(run_write);
    //    written += run_write;
    //}
...
size_t Ringbuffer<T, buffer_size, fake_tso, cacheline_size, index_t>::readBuff(T* buff, size_t count)
{
    ...
    kmdf_std::atomic_signal_fence(kmdf_std::memory_order_release);
    tail.store<index_release_barrier>(tmp_tail);

    return to_read;
    //index_t available = 0;
    //index_t tmp_tail = tail.load<kmdf_std::memory_order_relaxed>();
    //int to_read = static_cast<int>(count);

    //available = head.load<index_acquire_barrier>() - tmp_tail;

    //if (available < count) // do not read more than we can
    //    to_read = static_cast<int>(available);

    //size_t buffer_offset = tmp_tail % buffer_size;
    //size_t bytes_read = 0;
    //while (to_read > 0)
    //{
    //    size_t run_read = min(to_read, buffer_size - buffer_offset);
    //    RtlCopyMemory(buff + bytes_read, data_buff + buffer_offset, run_read);
    //    buffer_offset = (buffer_offset + run_read) % buffer_size;
    //    to_read -= static_cast<int>(run_read);
    //    bytes_read += run_read;
    //}
