...
public:
	...
	static NTSTATUS Create(PUNKNOWN* Unknown, 
						   REFCLSID, 
	...
	*/
	static NTSTATUS PropertyHandler_WaveFilter(_In_ PPCPROPERTY_REQUEST PropertyRequest);	IAdapterCommon* GetAdapter();

	NTSTATUS StreamClosed(ULONG pin, MiniportWaveRTStream* stream);
	NTSTATUS StreamCreated(ULONG pin, MiniportWaveRTStream* stream);

	BOOL IsSystemCapturePin(ULONG nPinId);
	BOOL IsBridgePin(ULONG nPinId);

	void writeToAllStreams(BYTE* buffer, ULONG packetSize, BOOL eos);
