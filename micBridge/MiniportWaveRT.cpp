...
--*/
{
	...
	*OutStream = NULL;
	...
	// Instantiate a stream. Stream must be in NonPagedPool(Nx) because of file saving.
	if (NT_SUCCESS(ntStatus))
	{
...
)
{
	...
	if (IsSystemCapturePin(_Pin))
	{
		...
		DPF(D_TERSE, ("MIC: Created %u th system capture stream.", m_ulSystemAllocated));
	}
	else 
	{
		DPF(D_TERSE, ("MIC: Created pin %n stream with type: %n.", _Pin, (int)GetPinTypeForPinNum(_Pin)));
	}
...
{
	PAGED_CODE();
	UNREFERENCED_PARAMETER(_Capture);
	UNREFERENCED_PARAMETER(_Pin);
	ASSERT(_Capture);
	ASSERT(IsSystemCapturePin(_Pin));

	return VerifyPinInstanceResourcesAvailable(m_ulSystemAllocated, m_ulMaxSystemStreams);;
}
...
void MiniportWaveRT::writeToAllStreams(BYTE* buffer, ULONG packetSize, BOOL eos)
{
	PAGED_CODE();
	...
	for (ULONG i = 0; i < m_ulMaxSystemStreams; ++i)
	{
		...
		{
			m_SystemStreams[i]->WriteAudioPacket(buffer, packetSize, eos);
		}
	}
}